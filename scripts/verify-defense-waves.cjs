...
(async()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),'pvz-waves-live-')),browser=await chromium.launch();try{
 for(const [name,options] of [['desktop',{viewport:{width:1440,height:900}}],['phone',{...devices['iPhone 13']}],['landscape',{...devices['iPhone 13'],viewport:{width:844,height:390}}]]){
  ...
  const geo=await page.evaluate(()=>{const board=document.getElementById('board').getBoundingClientRect(),tray=document.querySelector('.cards-panel').getBoundingClientRect();return{bottom:board.bottom,tray:tray.top,height:board.height,width:board.width,viewport:innerWidth}});expect(geo.width).toBeGreaterThan(geo.viewport*.8);expect(geo.bottom).toBeLessThanOrEqual(geo.tray);expect(geo.height).toBeGreaterThan(60);
