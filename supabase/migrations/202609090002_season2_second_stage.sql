-- Second-season stage two: preserve all existing progress/economy.
begin;
...
begin
 ...
 foreach side in array array['plants','zombies'] loop
  ...
  foreach k in array case when side='plants' then array['s2Tuntian','s2Crossbow','s2Shield','s2Halberd'] else array['s2Rat','s2Nail','s2Coffin','s2Cleaver'] end loop
...
begin
 ...
 select * into v_row from public.sgz_profiles where user_id=v_uid for update;
 if not found or v_row.active_device_id is distinct from p_device_id or v_row.active_seen_at is null or v_row.active_seen_at<=now()-interval '120 seconds' then raise exception 'DEVICE_LOCKED';end if;
 if p_level is null or p_level not in (1,2) then raise exception 'LEVEL_LOCKED';end if;
...
begin
 ...
 if v_match.season_no=2 and (v_match.level_no not in (1,2) or v_match.level_no=2 and coalesce((v_row.profile#>>array['season2Progress',v_match.faction,'completedLevels','1'])::integer,0)<1) then raise exception 'LEVEL_LOCKED';end if;
 ...
 if v_match.season_no=2 then
  v_allowed:=case when v_match.faction='plants' then array['s2Tuntian','s2Crossbow','s2Shield','s2Halberd'] else array['s2Rat','s2Nail','s2Coffin','s2Cleaver'] end;
  ...
  if p_character_key in ('s2Halberd','s2Cleaver') and coalesce((v_row.profile#>>array['season2Progress',v_match.faction,'completedLevels','2'])::integer,0)<1 then raise exception 'INVALID_CHARACTER';end if;
