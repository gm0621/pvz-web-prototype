...
test('season-two preview has two ten-stage plans, valid independent unlocks and no gameplay writes',async({page})=>{
 ...
 const check=await page.evaluate(()=>{
  ...
  return p.stages.length===10&&new Set(p.stages.map(s=>s.name)).size===10&&p.stages.every(s=>s.cardArt===null)&&season2Available('attack',11).length===0&&season2Available('defense',0).length===0;
 });
